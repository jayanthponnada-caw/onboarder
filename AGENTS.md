...
- `type` must be lowercase. Prefer `feat`, `fix`, `docs`, `refactor`, `test`, `build`, `ci`, `chore`, or `perf`.
- `scope` should be the smallest meaningful package, app, or concern, such as `web`, `core`, `infra`, `db`, `api-contract`, `docs`, or `repo`.
- `summary` should be imperative, present tense, and under 72 characters when practical.
- Use a detailed body for multi-package or architectural changes. Explain what changed and why, then list validation commands.
- Do not mix unrelated work in one commit when it can be cleanly split.

Examples:

```txt
feat(repo): scaffold onboarder monorepo
